...
/* Taken directly from the Thermo/src/hmi_hardware/synergy_graphics_driver_565rgb.c file,
   just removed the GUI bits - mikemac
*/
...
/* Ilitek9341.h and this big chunk of code for setting it up is taken from the 
   GUIX_template_WeatherPanel example program on https://synergygallery.renesas.com/
   */

#include "Ilitek9341.h"
...
void ILI9341V_Init(void)
{
    ...
    tx_semaphore_create (&g_hmi_semaphore_lcdc, (CHAR *) "New Semaphore", 0);

    err = g_ioport_on_ioport.pinWrite(LCD_CS,IOPORT_LEVEL_HIGH);
    err = g_ioport_on_ioport.pinWrite(LCD_RESET,IOPORT_LEVEL_HIGH);
    err = g_ioport_on_ioport.pinWrite(LCD_RESET,IOPORT_LEVEL_LOW);
    tx_thread_sleep(1);
    err = g_ioport_on_ioport.pinWrite(LCD_RESET,IOPORT_LEVEL_HIGH);
    if (SSP_SUCCESS != err)
    {
